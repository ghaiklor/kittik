...
describe("basic shape", () => {
  ...
  it("should properly get/set width", () => {
    ...
    shape.width = "50%";
    expect(parseInt(shape.width, 10)).toBe(
  ...
  it("should properly get/set height", () => {
    ...
    shape.height = "50%";
    expect(shape.height).toStrictEqual(
      Math.floor(process.stdout.rows / 2).toString()
    );
  });
  ...
  it("should properly get/set x coordinate", () => {
    ...
    shape.x = "center";
    expect(shape.x).toStrictEqual(
      Math.floor(
        process.stdout.columns / 2 - parseInt(shape.width, 10) / 2
      ).toString()
    );

    shape.x = "right";
    expect(shape.x).toStrictEqual(
      Math.floor(process.stdout.columns - parseInt(shape.width, 10)).toString()
    );

    shape.x = "50%";
    expect(shape.x).toStrictEqual(
      Math.floor(process.stdout.columns / 2).toString()
    );
  });
  ...
  it("should properly get/set y coordinate", () => {
    ...
    shape.y = "middle";
    expect(shape.y).toStrictEqual(
      Math.floor(
        process.stdout.rows / 2 - parseInt(shape.height, 10) / 2
      ).toString()
    );

    shape.y = "bottom";
    expect(shape.y).toStrictEqual(
      Math.floor(process.stdout.rows - parseInt(shape.height, 10)).toString()
    );
  ...
  it("should properly create Shape instance from JSON representation", () => {
    ...
    const json =
      '{"type":"Shape","options":{"text":"test","width":"30","height":"50","x":"1","y":"1"}}';
