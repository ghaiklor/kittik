...
export class Deck extends EventEmitter {
  ...
  public addShape(name: string, shape: ShapeRenderable): void {
    const slidesWithShape = this.slides
      .filter((slide) => slide.shapes.has(name))
    ...
    if (slidesWithShape.length > 0) {
      throw new Error(
        `You are trying to add a shape with the name "${name}" into the deck. ` +
          ...
          `Remove the shape from the slides [${slidesWithShape.join(
            ", "
          )}] or rename the shape.`
      );
    }
    ...
    this.slides.forEach((slide) => {
      slide.addShape(name, shape);
    });
  }

  public addAnimation(name: string, animation: Animationable): void {
    const slidesWithAnimation = this.slides
      .filter((slide) => slide.animations.has(name))
      .map((slide) => slide.name);
    if (slidesWithAnimation.length > 0) {
      throw new Error(
        `You are trying to add an animation with the name "${name}" into the deck. ` +
          ...
          `Remove the animations from the slides [${slidesWithAnimation.join(
            ", "
          )}] or rename the animation.`
      );
    }

    this.slides.forEach((slide) => {
      slide.addAnimation(name, animation);
    });
  }

  public addSlide(slide: Slide): void {
    if (
      this.slides.some((existingSlide) => existingSlide.name === slide.name)
    ) {
      throw new Error(
  ...
  private initSlides(declaration: DeckDeclaration): void {
    ...
    declaration.slides.forEach((slide) => {
      this.addSlide(
        Slide.create(
          {
            ...slide,
  ...
  private onKeyPress(chunk: string): void {
    switch (chunk) {
      case "p":
        this.previousSlide().catch((error) => {
          console.error(error);
        });
        break;
      case "n":
        this.nextSlide().catch((error) => {
          console.error(error);
        });
        break;
      case "q":
