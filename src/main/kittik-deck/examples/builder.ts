...
const PREDEFINED_SHAPES = {
  "Predefined Shape Example": ShapeBuilder.start("Rectangle")
    .withX("left")
    .withY("middle")
    .withBackground("aqua")
    .withForeground("blue")
...
const PREDEFINED_ANIMATIONS = {
  "Predefined Animation Example": AnimationBuilder.start("Slide")
    .withOptions({ direction: "inLeft" })
    .end(),
};

DeckBuilder.start(PREDEFINED_SHAPES, PREDEFINED_ANIMATIONS)
  .withCanvas(canvas)
  .withSlide((builder) =>
    builder
      .withName("Slide #1")
  ...
  .withSlide((builder) =>
    builder
      ...
      .withAnimation(
        ...
        AnimationBuilder.start("Print").withDuration(5000).end()
      )
      .withOrder("Predefined Shape Example", ["Predefined Animation Example"])
