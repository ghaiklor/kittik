...
type TShapeAccumulator<TSlideBuilder, TNextShape> =
  TSlideBuilder extends SlideBuilder<infer TShapes, infer TAnimations>
    ? SlideBuilder<TNextShape | TShapes, TAnimations>
    : never;
...
export class SlideBuilder<TShape, TAnimation> {
  ...
  public constructor(
    ...
    Object.keys(shapes).forEach((name) => {
      ...
      this.slide.addShape(name, shapes[name]!);
    });

    Object.keys(animations).forEach((name) => {
      // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
      this.slide.addAnimation(name, animations[name]!);
    });
  }
  ...
  public withShape<T extends string>(
    name: T,
    shape: ShapeRenderable
  ): TShapeAccumulator<this, T> {
  ...
  public withAnimation<T extends string>(
    name: T,
    animation: Animationable
  ): TAnimationAccumulator<this, T> {
