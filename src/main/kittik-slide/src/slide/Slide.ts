...
export class Slide {
  ...
  public addOrder(shape: string, animations: string[] = []): void {
    ...
    const unknownAnimations = animations.filter(
      (animation) => !this.animations.has(animation)
    );
  ...
  public async render(): Promise<void> {
    ...
    const onTick = (): void => {
      this.renderShapes(shapesToRender);
    };
    ...
    for (const order of this.order) {
       ...
       */
      (order.animations?.map((item) => animations.get(item)) ?? []).forEach(
        (animation) => sequence.push(() => animation?.animate(shapeToRender))
      );
    ...
    sequence.push(() => {
      animations.forEach((animation) =>
        animation.removeListener("tick", onTick)
      );
    });
     ...
     */
    await sequence.reduce(async (promise, item) => {
  ...
  public toObject(): SlideDeclaration {
    ...
    const shapes = [...this.shapes.entries()].map(([shapeName, shape]) => ({
      ...shape.toObject(),
      name: shapeName,
    }));

    const animations = [...this.animations.entries()].map(
  ...
  private initAnimations(declaration: AnimationDeclaration[]): void {
    declaration.forEach((animationDeclaration) => {
      ...
      this.addAnimation(
        animationDeclaration.name,
        ctor.fromObject(animationDeclaration)
      );
    });
  }
